...
# Data cleaning project for employee payrollment 
# Task 1
...
# Task 2
# Finding the lowest department number encountered the 'dept_emp' table. Then, find the highest department number.
SELECT
    MIN(dept_no)
FROM
    dept_emp;
...
# Task 3
# To obtain a table containing the following three fields for all individuals whose employee number is no greater than 10040:
# - employee number
...
# Task 4
# Retrieving a list with all employees that have been hired in the year 2000.
SELECT
    *
FROM
    employees
...
# Task 5
# Retrieving a list with all employees from the ‘titles’ table who are engineers. 
# Repeating the exercise, this time retrieving a list with all employees from the ‘titles’ table who are senior engineers.
SELECT
    *
FROM
    titles
...
# After LIKE, we could proceed to indicate what you are looking for with or without using parentheses. Both options work correctly. We think using parentheses is better for legibility reasons and that’s why it is the first option we’ve suggested.
SELECT
    *
FROM
    titles
...
# Task 6
# Creating a procedure that asks us to insert an employee number to obtain an output containing the same number, as well as the number and name of the last department the employee has worked for.
# Finally, we call the procedure for employee number 10010.
# If we've worked correctly, we should see that employee number 10010 has worked for department number 6 - "Quality Management".
...
# Hint: we may wish to compare the difference between the start and end date of the salaries contracts.
SELECT 
    COUNT(*)
FROM
    salaries
...
# Task 8
# Creating a trigger that checks if the hire date of an employee is higher than the current date. If true, set this date to be the current date. We format the output appropriately (YY-MM-DD).
# Extra challenge: We may try to declare a new variable called 'today' which stores today's data, and then use it in your trigger!
# After creating the trigger, we execute the following code to see if it's working properly.
/*
...
# Task 9
# Defining a function that retrieves the largest contract salary value of an employee. Apply it to employee number 11356. 
# Also, what is the lowest salary value per contract of the same employee? We may want to create a new function that will deliver this number to you.  Apply it to employee number 11356 again.
...
# Task 10
# Based on the previous example, we can now try to create a function that accepts also a second parameter which would be a character sequence. 
